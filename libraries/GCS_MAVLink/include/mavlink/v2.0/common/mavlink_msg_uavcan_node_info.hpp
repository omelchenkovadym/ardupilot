 ...
 * General information describing a particular UAVCAN node. Please refer to the definition of the
                ...
                emitted by the system whenever a new node appears online, or an existing node reboots. Additionally, it
                can be emitted upon request from the other end of the MAVLink channel (see
                MAV_CMD_UAVCAN_GET_NODE_INFO). It is also not prohibited to emit this message unconditionally at a low
...
struct UAVCAN_NODE_INFO : mavlink::Message {
    ...
    uint32_t sw_vcs_commit; /*<  Version control system (VCS) revision identifier (e.g. git short
                commit hash). Zero if unknown.
             */
